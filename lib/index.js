...
const nodeFsLite = {
    ...
    ensureEmptyDirSync(p) {
        gracefulSync(fs.rmSync, p, { recursive: true, force: true });
        gracefulSync(fs.mkdirSync, p, { recursive: true });
        return p;
    },
    ...
    emptyDirSync(p) {
        ...
        const list = gracefulSync(fs.readdirSync, p);
        for (const item of list) {
            const full = path.join(p, item);
            gracefulSync(fs.rmSync, full, { recursive: true, force: true });
        }
        return p;
    },
    ...
    async ensureEmptyDir(p) {
        await gracefulAsync(fsPromises.rm, p, {
            recursive: true,
            force: true,
        });
        await gracefulAsync(fsPromises.mkdir, p, {
            recursive: true,
        });
        return p;
    },
    ...
    async emptyDir(p) {
        ...
        const list = await gracefulAsync(fsPromises.readdir, p);
        for (const item of list) {
            const full = path.join(p, item);
            await gracefulAsync(fsPromises.rm, full, {
                recursive: true,
                force: true,
            });
        }
